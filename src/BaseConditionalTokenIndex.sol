...
abstract contract BaseConditionalTokenIndex is ERC20, IERC1155Receiver, ERC165 {
    ...
    function redeem() external {
        ...
        if (!indexSolved) revert IndexNotSolvedYet();
    ...
    function _deposit(uint256 amount) internal virtual {
        console.log("   [BaseConditionalTokenIndex]1. _deposit :", msg.sender);
        uint256 len = components().length;
        console.log("   [BaseConditionalTokenIndex]2. components length:", len);
        uint256[] memory amts = new uint256[](len);
        console.log("   [BaseConditionalTokenIndex]3. amts length:", amts.length);
        unchecked {
            for (uint256 i = 0; i < len; ++i) {
                amts[i] = amount;
            }
        }
        console.log("   [BaseConditionalTokenIndex]4.");
        _mint(msg.sender, amount);
        console.log("   [BaseConditionalTokenIndex]5. mint done");
        uint256[] memory componentsArray = components();
        ctf().safeBatchTransferFrom(msg.sender, address(this), componentsArray, amts, "");
        console.log("   [BaseConditionalTokenIndex]6. transfer done");
    }
     ...
     */
    function canSolveIndex() external view returns (bool canSolve) {
